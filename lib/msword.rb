require 'redcloth'
module RedCloth::Formatters::MSWordScript
  ...
  [:code, :em, :i, :b, :ins, :sup, :sub, :span, :cite].each do |m|
    define_method(m) do |opts|
      opts[:block] = true
      "<#{m}#{pba(opts)}>#{opts[:text]}</#{m}>"
    end
...
module RedCloth
  class TextileDoc < String
    def to_wordscript( *rules )
      apply_rules(rules)
  
      to(RedCloth::Formatters::MSWordScript)
    end
	end
end

# =>"<p>And then? She <st>fell<st>!</p>"
puts RedCloth.new( "And then? She *fell*!" ).to_wordscript
